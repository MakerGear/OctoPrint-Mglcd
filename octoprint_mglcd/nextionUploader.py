...
def getBaudrate(ser, fSize=None, checkModel=None):
	for baudrate in (115200, 2400, 4800, 9600, 19200, 38400, 57600):
		...
		print('Trying with baudrate: {}...'.format(str(baudrate)))
		ser.write(e)
		ser.write(b'connect')
		ser.write(e)
		r = ser.read(128)
		if 'comok' in r:
			print('Connected with baudrate: {}...'.format(str(baudrate)))
			ser.write(b'bauds=115200')
			ser.write(e)
			baudrate = 115200

			noConnect = False
			status, unknown1, model, fwversion, mcucode, serial, flashSize = r.strip("\xff\x00").split(',')
			print('Status: {}'.format(status.split(' ')[0]))
			if status.split(' ')[1] == "1":
			...
			print('MCU code: {}'.format(mcucode))
			print('Serial: {}'.format(serial))
			print('Flash size: {}'.format(flashSize))
...
def setDownloadBaudrate(ser, fSize, baudrate):
	ser.write(b"")
	ser.write(b"whmi-wri {},{},0{}".format(str(fSize), str(baudrate), e))
	time.sleep(.05)
	ser.baudrate = baudrate
	ser.timeout = .5
	r = ser.read(1)
...
if __name__ == "__main__":
	...
	try:
		...
		print('could not open serial device {}'.format(sys.argv[2]))
		exit(1)
